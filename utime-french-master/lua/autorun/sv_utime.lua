...
function onJoin( ply )
	...
	if row then
		if utime_welcome:GetBool() then
			ULib.tsay( ply, "[Utime]Content de te revoir " .. ply:Nick() .. ", vous avez joué pour la dernière fois sur ce serveur " .. os.date( "%c", row.lastvisit ) )
		end
		sql.Query( "UPDATE utime SET lastvisit = " .. os.time() .. " WHERE player = " .. uid .. ";" )
		time = row.totaltime
	else
		if utime_welcome:GetBool() then
			ULib.tsay( ply, "[Utime]Bienvenue sur notre serveur " .. ply:Nick() .. "!" )
		end
